...
- [store — 本地缓存存储引擎](#store--本地缓存存储引擎)
  ...
  - [6. 单元测试覆盖](#6-单元测试覆盖)
    - [store_test.go（6 个测试）](#store_testgo6-个测试)
    - [lru_test.go（18 个测试）](#lru_testgo18-个测试)
    - [lru2_test.go（20 个测试）](#lru2_testgo20-个测试)
  - [7. 使用示例](#7-使用示例)
...
Get(key)
  ...
  │
  └─② expires[key] 过期检查 → 已过期 → 异步 Delete → 返回 (nil, false)
       │
       └─③ list.MoveToBack(elem) → 返回 (value, true)
```

- **步骤①**：通过 `items` 哈希表 O(1) 定位链表节点，无需遍历链表。
- **步骤②**：通过独立的 `expires` 映射 O(1) 检查过期。过期数据不立即同步删除，而是**异步调用 `Delete`** 避免阻塞当前请求。
...
| 机制         | 触发时机                     | 行为                                         |
| ------------ | ---------------------------- | -------------------------------------------- |
| **惰性删除** | `Get` 时发现已过期           | 异步 goroutine 调用 `Delete`，不阻塞当前请求 |
